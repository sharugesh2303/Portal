...
// --- Helper Variables ---
const WATERMARK_PATH = 'public/jjcet_watermark.png'; 
const HEADER_IMAGE_PATH = 'public/jjcet_logo.jpg'; 
...
// --------------------------------------------------------------------------------------------------
// --- Helper: Draw Watermark (2% Opacity, Center Placement) ---
// --------------------------------------------------------------------------------------------------
...
        
        const x = (pageWidth / 2) - (watermarkSize / 2);
        const y = (pageHeight / 2) - (watermarkSize / 2); 

        // Opacity 0.02 (2%): The lowest reliable visual setting.
        doc.image(WATERMARK_PATH, x, y, { 
            width: watermarkSize, 
            opacity: 0.02 
        });
        
...
        doc.image(HEADER_IMAGE_PATH, margin, yStart, { width: doc.page.width - (2 * margin), height: headerHeight, fit: [doc.page.width - (2 * margin), headerHeight], align: 'center', valign: 'center' });
        textY += headerHeight;
    } else {
        doc.fillColor(blackColor).font('Helvetica-Bold').fontSize(12).text(`--- HEADER IMAGE MISSING: Check that the file 'jjcet_logo.jpg' is in your backend's public/ folder ---`, 0, yStart + 35, { align: 'center' });
        textY += 60;
    }
...
// --------------------------------------------------------------------------------------------------
// --- Helper: Payslip Generator (FINAL FIX) ---
// --------------------------------------------------------------------------------------------------
...
            const deductionValue = deductions[i].value; 
            doc.text(formatCurrency(deductionValue), col4X, currentY + 4, { width: 80, align: 'right' });
...
    
    doc.text('Gross Pay', startX, currentY);
    doc.text(formatCurrency(grossPay), col2X + 20, currentY, { width: 80, align: 'right' });
    
    doc.text('Total Deductions', startX + halfWidth + 5, currentY); 
    doc.text(formatCurrency(totalDeductions), col4X, currentY, { width: 80, align: 'right' }); 
    currentY += rowHeight;
...
    // 7. Signature Placeholder (FINAL, VISIBLE SIGNATURE BLOCK)
    currentY += 50; 
    // Set color to black for text
    doc.fillColor(blackColor).font('Helvetica-Bold').fontSize(12).text('PRINCIPAL', startX + tableWidth - 120, currentY); 
    
    // Removing the signature line:
    // doc.strokeColor(blackColor).lineWidth(0.5)
    //    .moveTo(startX + tableWidth - 120, currentY + 15) 
    //    .lineTo(startX + tableWidth, currentY + 15) 
    //    .stroke();
...
        // FIX APPLIED: Iterate through records and generate each payslip
...
    } catch (err) {
        console.error('Error generating annual payslip report.', err);
...
    } catch (err) {
        console.error('Error generating monthly payslip report.', err);
